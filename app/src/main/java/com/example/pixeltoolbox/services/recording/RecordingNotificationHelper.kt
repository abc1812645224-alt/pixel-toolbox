...
import com.example.pixeltoolbox.ui.theme.Green40
...
class RecordingNotificationHelper(private val context: Context) {
    ...
    fun createNotificationChannels() {
        ...
        val group = NotificationChannelGroup(groupId, "通话录音")
        manager.createNotificationChannelGroup(group)

        // Recording Service Channel
        val serviceChannel = NotificationChannel(
            CHANNEL_ID_SERVICE, "前台录音服务", NotificationManager.IMPORTANCE_HIGH
        ).apply {
            this.group = groupId
        ...
        val errorChannel = NotificationChannel(
            CHANNEL_ID_ERROR, "录音错误", NotificationManager.IMPORTANCE_HIGH
        ).apply {
            this.group = groupId
        ...
        when (newState) {
            is RecordingServiceState.Standby  -> {
                if (newState.metadata == null) {
                    ...
                    val metadata = newState.metadata
                    if (metadata != null) {
                        val dirLabel = metadata.direction.labelResId.let { context.getString(it) }
