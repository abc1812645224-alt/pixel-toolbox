...
package com.example.pixeltoolbox.data.call
...
data class EnrichedCallData(
    ...
    companion object {
         ...
         */
        const val EXTRA_METADATA = "com.example.pixeltoolbox.EXTRA_RECORDING_METADATA"
        ...
        suspend fun enrichMetadata(context: Context, base: RawCallData): EnrichedCallData =
            withContext(Dispatchers.Default) {
                ...
                // Ported without PhoneNumberManager/libphonenumber: perform basic normalization only.
                ...
                AppLogger.v( "Enriched metadata for number: raw='$raw', callerName='$callerName'")
                return@withContext EnrichedCallData(
                    normalisedPhoneNumber = normalisePhoneNumber(raw), // Safety normalizing, the number should already have been.
                    formattedE164Number = null,
                    direction = base.direction,
         ...
         */
        /**
         * Basic phone-number normalization (ported from PhoneNumberManager to avoid libphonenumber dependency).
         */
        private fun normalisePhoneNumber(phoneNumber: String): String {
