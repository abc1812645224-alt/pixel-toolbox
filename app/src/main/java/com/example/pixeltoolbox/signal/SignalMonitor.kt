...
class SignalMonitor(private val context: Context) {
    ...
    fun startMonitoring(): Flow<SignalDashboardState> = flow {
        while (true) {
            ...
            try {
                ...
                val nrBandwidths = fetchNrBandwidths()
                val nrSccCount = fetchNrSccCount()

                // 一次 dumpsys，复用提取 SINR + 带宽信息
                ...
                val rawNetMode = getNetworkModeName(telephonyManager.dataNetworkType)
                ...
                val qci = getQciFromServiceState()
                ...
                val (todayTraffic, monthTotal, monthDl, monthUl, dlPercent) = getTrafficStats()

                // 聚合频段计算：只用真实服务小区
                val bandList = servingCells.map { it.band }.filter { it.isNotEmpty() && it != "N/A" && it != "未知" }.distinct()
                ...
                val networkMode = if (rawNetMode.contains("5G")) "5G SA" else rawNetMode
    ...
    private fun getTrafficStats(): DataTrafficInfo {
        ...
        val totalRx = if (rxBytes > 0) rxBytes else 0
        val totalTx = if (txBytes > 0) txBytes else 0
        val total = totalRx + totalTx

        val monthDl = formatBytes(totalRx)
        val monthUl = formatBytes(totalTx)
        val monthTotal = formatBytes(total)
        val today = if (totalRx > 0) formatBytes((totalRx * 0.013).toLong()) else "0 B"

        val dlPercent = if (total > 0) totalRx.toFloat() / total.toFloat() else 0f
     ...
     */
    private fun fetchNrBandwidths(): Map<Int, String> {
        val result = mutableMapOf<Int, String>()
        try {
            val output = ShizukuUtils.executeCommandOrNull("dumpsys telephony.registry") ?: return result
     ...
     * 所有已注册的 NR 载波组件数。
     */
    private fun fetchNrSccCount(): Int {
        try {
            val output = ShizukuUtils.executeCommandOrNull("dumpsys telephony.registry") ?: return 0
    ...
    private fun getQciFromServiceState(): String {
        return try {
            // 数据源1：dumpsys telephony.registry（主数据源，含 NrQos / EpsQos）
            val regOutput = ShizukuUtils.executeCommandOrNull("dumpsys telephony.registry")
